...
def _sanitize_input(text: str) -> str:
    ...
    logger.info("MOA: Sanitizing input to remove code blocks")
...
def mixture_of_agents(
    ...
    # Sanitize input to prevent code blocks from causing errors
    sanitized_query = _sanitize_input(initial_query)

    logger.debug(
        f"Generating initial completions for query: {sanitized_query[:100]}..."
    )
    ...
    try:
        ...
        # Check if this is a validation error (400) - should fail fast
        if hasattr(e, "status_code") and e.status_code == 400:
            logger.error(f"Provider validation error in MOA: {str(e)}")
        ...
        for i in range(3):
            try:
                ...
                if (
                    hasattr(fallback_error, "status_code")
                    and fallback_error.status_code == 400
                ):
                    logger.error(f"Validation error in completion {i+1}: {last_error}")
                    break  # Don't continue if validation fails
                # For other errors (rate limit, server error), continue trying
                logger.error(f"Error generating completion {i+1}: {last_error}")
                continue

        if not completions:
            error_msg = (
                f"Could not generate any completions. Last error: {last_error}"
                if last_error
                else "Could not generate any completions"
            )
